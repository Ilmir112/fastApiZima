...
async def add_wells_data(
    ...
    try:
        if brigade_info:
            await delete_brigade(number_brigade=brigade_info.number_brigade, contractor=brigade_info.contractor)
...
async def delete_brigade(
        brigade: BrigadeSearch = Depends(),
        user: Users = Depends(get_current_user)):
    data = await find_brigade_one(number_brigade=brigade.number_brigade, contractor=brigade.contractor)
    if data:
