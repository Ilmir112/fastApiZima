...
class NormsWork(Base):
    ...
    type_tkrs: str = Column(String, nullable=False)
    summary_work: dict = Column(JSON, nullable=False)
    norms_json: dict = Column(JSON, nullable=False)
    creater_id: int = Column(Integer, ForeignKey("users.id"))
    lifting_unit: str = Column(String, nullable=False)
    number_brigade: str = Column(String, nullable=False)
