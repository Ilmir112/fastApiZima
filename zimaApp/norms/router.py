...
async def find_well_filter_by_number(
    ...
    try:
        ...
        }  # Возвращайте осмысленную ошибку
...
async def find_norms_one(
    ...
    result = await NormDAO.find_one_or_none(wells_id=wells_repair.wells_id)
    return result
...
@version(1)
async def find_norms_one(
    norms: NormSearchId = Depends(), user: Users = Depends(get_current_user)
):
    result = await NormDAO.find_one_or_none(id=norms.id)
    return result
...
async def add_norm_data(
    ...
    try:
        if norms and repair_wells:
            ...
            if result:
                await TelegramInfo.send_message_create_norms(
                    user.login_user, norms.repair_id
                )
...
async def update_norms_data(
    ...
    try:
        if norms_id:
            result = await NormDAO.update_data(
                norms_id.id,
                repair_well_repair=norms.repair_well_repair,
                summary_work=norms.summary_work,
                norms_json=norms.norms_json,
            ...
            await TelegramInfo.send_message_update_norms(
                user.login_user, norms_id.id, result.number_brigade
            )
...
async def delete_norms(norms: SNorms, user: Users = Depends(get_current_user)):
    data = await NormDAO.find_one_or_none(
        repair_id=norms.repair_id, start_well_repair=norms.start_well_repair
    )
    if data:
        return await NormDAO.delete_item_all_by_filter(
            repair_id=norms.repair_id, start_well_repair=norms.start_well_repair
        )
